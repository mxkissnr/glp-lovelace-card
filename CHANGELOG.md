...
- **The ready-by scheduler's "Setzen" button and the maintenance confirm-yes button still used a 100%-opacity solid `var(--green)` fill with dark text** — the loud, saturated CTA style the redesign replaced everywhere else with a tinted fill (`color-mix(... 12-14%, transparent)`) plus colored text, e.g. `.maint-pill.ok`. Both predate #120 (`.ready-by` is #61) and were never touched by it. `.ready-by-btn.primary` and `.maint-confirm-yes` now match that established pattern. glp-lovelace-card#125
...
### Changed
- **"Instrument" redesign applied to the card itself** (#120), built on the token/icon groundwork already landed below. Uppercase + letter-spacing removed from all 8 labels that had it — sentence case, small, muted via `--glp-sub` — and every `font-size` literal now resolves through `--glp-fs-1..6` (28 distinct ad-hoc values, several below the new floor, down to 6 token references) with gap/padding pulled onto the `--glp-sp-1..6` ladder. The two three-tile box rows (`.metric-trio` for a historical shot's duration/yield/ratio, `.live-stats` for a brewing shot's temp/pressure/weight) — independently flagged as the single clearest "generated UI" tell in this file — collapse into one shared `.metric-line` component (`metricLineHtml()`), with the three roles (recipe/process/result) told apart by typographic weight and size, not by giving each its own box. The score ring badge (a 54px circle with a coloured ring) is now a typographic verdict — `"87 · good"`, sans throughout — with new `verdict_high`/`verdict_mid`/`verdict_low` strings in all six languages; the underlying score thresholds are unchanged, only the presentation is. All 88 emoji glyphs (STRINGS across six languages, `MAINT_TASKS`, the ★ rating stars, `⚙️`/`🔌`/`✕` render-path glyphs) replaced by the shared `ICONS` drawn-icon set added below; `flagEmoji()` is dropped without replacement (the country name renders right next to it and the flag added no information the OS didn't already vary). Border diet: full 1px borders now only around clickable elements — static/informational groupings (live-machine tiles, maintenance rows without a confirm flow, banners, the ready-by/profile-picker wrappers) group via surface fill instead, which also flattens the three-levels-deep card-in-card nesting the profile picker and ready-by wrapper had. New `--glp-aline` (added below) now actually has two callers: the active tab's underline and the active shot's nav dot. The historical shot chart draws itself in on load — pressure/flow/temp/weight stroke in staggered from the left, the preinfusion/extraction shading fades in behind them, a weight endpoint marker last, ~1.5s total — gated by a new pure `_shotChartKeyChanged()` so it plays once per actual shot change and not on every incidental re-render (same problem the nav-dot "changed" tracking already solved for the dots); the live brewing chart never animates, since it redraws on every incoming datapoint. `prefers-reduced-motion` now covers all four animated effects in the file instead of one (`.lm-live-dot` only, previously) — the brewing status-dot pulse, the nav-dot grow, and the new curve draw-in are pinned to their finished state, not merely stopped mid-animation. Fraunces 600 is scoped to the bean name (`.shot-coffee`) alone, via the platform serif fallback stack — no font file is bundled in this single-file custom element, unlike the app. HA theme tokens (`--glp-bg/-surface/-border/-text`) are untouched by this pass; the redesign works through typography, rhythm and iconography here, not a hardcoded palette. `glp-card.js`, `test/curve-animation.test.js` (new). Closes #120
### Added
- **Type scale, spacing ladder and a contrast-corrected accent line token in the shared `GLP-TOKENS v1` block** — the token groundwork for the "Instrument" redesign (#120, alongside glp-order-card#90 and gaggiuino-local-profiler#811). `--glp-fs-1..6` replaces a tail of 28 distinct ad-hoc font-sizes stepping in 0.02rem increments, `--glp-sp-1..6` replaces the loose gap/padding literals. The smallest step is deliberately 0.8125rem, not the 0.5-0.6rem the card used to reach for: the redesign's border diet removes boxes as a grouping device and must not bring them back as unreadable hairline micro-typography. Radii are deliberately not part of the ladder — the card already resolves every corner through `--glp-radius`/`--glp-radius-sm`, and `--glp-radius` stays HA-led so a card keeps matching the dashboard it sits on.
- **`--glp-aline`: the accent as a thin line, resolved at runtime to clear WCAG 1.4.11's 3:1 non-text contrast floor.** Three of the eight curated machine themes miss that as a line against a dark ground (Ruby Ristretto 1.88:1, Mulberry Mocha 2.09:1, Twilight Turkish 2.38:1) — a pre-existing gap that only becomes visible now that the redesign uses accent lines where it used to draw borders. New `_applyAccentLineContrast()` takes the darker gradient stop (same worst-case reasoning as `--glp-accent-text`) and blends it toward `--glp-text` in 5% steps until it clears 3:1; a theme that already passes is left byte-exact. **Fills are never touched** — `--glp-accent-start`/`-end` keep their configured hex values, so gradients, the preheat bar and the machine icon render exactly as before.
...
- **External GLP link in the card footer hardened against tabnabbing.** Added `rel="noopener noreferrer"` to the `target="_blank"` anchor; not an XSS risk since the URL comes from the card's own YAML config, not backend data. `glp-card.js`. Closes #100
- **Remaining GitHub Actions workflows pinned to commit SHA, explicit token permissions added.** Resolved 11 CodeQL/Scorecard findings: `validate.yml` and `codeql.yml` lacked a top-level `permissions: contents: read` block, and `actions/checkout`, `actions/setup-node`, `hacs/action`, `github/codeql-action/{init,analyze}`, and `actions/dependency-review-action` were referenced by floating tag instead of pinned SHA. `.github/workflows/codeql.yml`, `.github/workflows/dependency-review.yml`, `.github/workflows/validate.yml`. Closes #93
- **`brace-expansion` bumped to 5.0.9 (GHSA-rgw5-rvv9-x895).** Transitive dev dependency via ESLint's minimatch; the `maxLength` mitigation added in 5.0.8 for the prior DoS (CVE-2026-14257) was incomplete, 5.0.9 fixes the remaining unbounded intermediate-array path. Dev-only tooling, not part of the shipped card code. `package-lock.json`. Closes #92
### Documentation
- Added an AI-generated-project disclaimer to the README. `README.md`. Closes #98
### Dependencies
- `actions/dependency-review-action` 4.9.0 → 5.0.0, `actions/upload-artifact` 4.6.2 → 7.0.1, `playwright` (dev) 1.62.0 → 1.62.1.

## [2.18.0] – 2026-08-03
### Added
- **Per-machine colour theme (8 curated presets + custom flat colour/gradient) and a new detailed Gaggia Classic machine icon.** Companion to the GLP app's own machine-theme storage (`mxkissnr/gaggiuino-local-profiler#595`, `machines.theme`); this card has no direct database access, so it gets new optional `setConfig()` keys mirroring the same `{"preset":"<key>"}`/`{"a":"#rrggbb","b":"#rrggbb"}` shape as its standalone/no-app YAML fallback: `theme` (one of the 8 preset keys — `amber-americano`, `ruby-ristretto`, `copper-cortado`, `twilight-turkish`, `marbled-macchiato`, `ember-espresso`, `mulberry-mocha`, `frosty-flat-white`), `accent_color` (custom flat `#rrggbb`), and `accent_gradient` (custom `["#rrggbb","#rrggbb"]` two-stop gradient) — precedence `accent_gradient` > `accent_color` > `theme`, strictly hex-validated. Extends `GLP-TOKENS v1`'s single `--glp-accent` into `--glp-accent-start`/`--glp-accent-end` (both default to the previous single value, so an untheemed card is unchanged) and extends `GLP-SHARED:contrast v1` to pick `--glp-accent-text` off the darker of the two gradient stops. The new icon (ported faithfully from an approved mockup, geometry unchanged) renders in the resolved theme colour as a small badge in the card header, next to the title; each card instance gets its own unique SVG gradient id so more than one themed card can render on the same dashboard. The GLP app is the primary/only source once a card-to-app theme sync exists (a later round) — until then, this YAML config is the standalone fallback. `glp-card.js`, `test/machine-theme.test.js` (new), `test/semantic-color-contrast.test.js` (extended for the darker-stop rule), README. Closes #87
