...
- **Flat HA-theme redesign — hybrid tokens, decorative glow removed.** Purely visual/theming, no behavior change (one small bugfix called out below). Closes #56
  ...
  - **Follow-up fix:** `--glp-ok`/`--glp-warn`'s fixed values (`#22c55e`/`#eab308`) only cleared 2.28:1/1.92:1 against a white background — well under the 4.5:1 WCAG AA floor this card's small/bold badge, banner and star-rating text needs — visibly broken in the "Brühbereit" preheat banner, the live status dot ring and the star rating in `docs/screenshots/card-light.png`. Added a `@media (prefers-color-scheme: light)` override (`--glp-ok: #15803d` 5.02:1, `--glp-warn: #a16207` 4.92:1, both measured against white) inside the same GLP-TOKENS v1 block; deliberately does *not* chain through HA's own `--success-color`/`--warning-color` the way `--glp-err` chains through `--error-color`, because `glp-ha-theme.yaml`'s own "GLP Light" theme sets those to values that themselves only clear 3.30:1/3.19:1 — letting an arbitrary theme win here would still ship a contrast failure. `scripts/screenshot.mjs` now explicitly sets Playwright's `colorScheme` per run so the media query fires deterministically.
